...
export const EditTour = ({ tour, onClose, onEdited }) => {
  const [form, setForm] = useState({ title: "", description: "", price: "", duration: "", maxPeople: "", availableSlots: "" })
  ...
  useEffect(() => {
    if (!tour) return
    setForm({ title: tour.title || "", description: tour.description || "", price: tour.price || "", duration: tour.duration || "", maxPeople: tour.maxPeople || "", availableSlots: tour.availableSlots || "" })
    setSelectedLocations(tour.locations || (tour.location ? [tour.location] : []))
    setSchedule(tour.schedule?.length > 0 ? tour.schedule.map((s, i) => ({ day: i + 1, content: s.content })) : [{ day: 1, content: "" }])
  }, [tour])
      ...
      <div className="bg-white rounded-2xl p-6 w-[560px] shadow-xl my-auto">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <input name="duration" placeholder="Thời gian" className="w-full border rounded-lg px-3 py-2 text-sm" value={form.duration} onChange={handleChange} />
          ...
          <div>
            ...
            {tour?.images?.length > 0 && (
              <div className="flex gap-2 mb-2 flex-wrap">
                {tour.images.map((img, i) => <img key={i} src={img} className="w-14 h-14 rounded-lg object-cover border" />)}
              </div>
            )}
